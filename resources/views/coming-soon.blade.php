...
<head>
    ...
    <style>
        ...
        .glow-radial {
            background: radial-gradient(circle at 50% 30%, rgba(230, 23, 31, 0.25) 0%, rgba(15, 23, 42, 0) 70%);
        }
...
<body
    class="bg-slate-950 text-slate-100 min-h-full flex flex-col justify-between selection:bg-red-600 selection:text-white relative overflow-x-hidden"
    x-data="{
    ...
    <div class="fixed inset-0 pointer-events-none z-0">
        <div class="absolute inset-0 bg-cover bg-center opacity-15 mix-blend-luminosity filter blur-[1px]"
        ...
        <div class="absolute inset-0 glow-radial"></div>
        <div class="absolute inset-0 pattern-grid opacity-40"></div>
    </div>

    <!-- Header / Navbar -->
    <header class="relative z-10 w-full border-b border-white/10 bg-slate-950/60 backdrop-blur-xl">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3 sm:gap-6">
                <!-- Status Badge -->
                <div
                    class="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold">
                ...
                <!-- Language Selector -->
                <div class="flex items-center bg-slate-900/80 p-1 rounded-lg border border-white/10 shadow-inner">
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <a href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}"
                            class="px-2.5 py-1 text-xs font-bold rounded-md transition-all uppercase {{ app()->getLocale() === $localeCode ? 'bg-red-600 text-white shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5' }}">
                            {{ $localeCode }}
                        </a>
                    @endforeach
                </div>
    ...
    <main class="relative z-10 flex-grow flex items-center py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
        <div class="max-w-5xl mx-auto w-full text-center">
            ...
            <!-- Pill Label -->
            <div
                class="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-red-600/15 border border-red-500/30 text-red-400 text-xs sm:text-sm font-semibold mb-8 backdrop-blur-md">
            ...
            <h1 class="text-4xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight text-white mb-6 leading-tight">
                @if(app()->getLocale() === 'fr')
                    L’Excellence des <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-rose-400 to-amber-400">Sols &
                        Murs</span> B2B Arrive.
                @elseif(app()->getLocale() === 'ar')
                    قمة الاحترافية في <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-rose-400 to-amber-400">الأرضيات
                        والتكسيات</span> قريباً.
                @else
                    Next-Level <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-rose-400 to-amber-400">Flooring
                        & Walls</span> Launching Soon.
                @endif
            </h1>

            <!-- Subtitle Description -->
            <p class="text-base sm:text-xl text-slate-300 max-w-3xl mx-auto mb-10 leading-relaxed font-light">
                @if(app()->getLocale() === 'fr')
                    Nous préparons l'expérience digitale ultime pour les architectes, maîtres d'ouvrage et professionnels du
            ...
            <div
                class="inline-grid grid-cols-4 gap-2 sm:gap-4 p-3 sm:p-5 rounded-2xl bg-slate-900/70 border border-white/10 backdrop-blur-2xl shadow-2xl mb-12 max-w-xl mx-auto w-full">
                <!-- Days -->
                <div
                    class="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-950/60 border border-white/5">
                    <span class="text-2xl sm:text-4xl font-black text-white tracking-wider"
                        x-text="String(days).padStart(2, '0')">14</span>
                    <span class="text-[10px] sm:text-xs text-slate-400 font-medium uppercase mt-1">
                ...
                <div
                    class="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-950/60 border border-white/5">
                    <span class="text-2xl sm:text-4xl font-black text-red-400 tracking-wider"
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
                ...
                <a href="mailto:info@maxprosols.com"
                    class="w-full sm:w-auto px-8 py-4 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-200 hover:text-white font-semibold text-sm border border-white/10 hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-2.5">
            ...
            <!-- Core Pillars: 5 Specialties (Glassmorphism Cards) -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 text-left"
                dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                
                <!-- 01 — Carrelage & Faïence -->
                <div class="p-4 rounded-xl bg-slate-900/50 border border-white/5 hover:border-red-500/30 transition-colors backdrop-blur-sm group flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <div class="w-9 h-9 rounded-lg bg-red-600/10 border border-red-500/20 flex items-center justify-center text-red-400 group-hover:scale-110 transition-transform">
                        ...
                        <h3 class="font-bold text-xs text-white mb-1">
                ...
                <div class="p-4 rounded-xl bg-slate-900/50 border border-white/5 hover:border-red-500/30 transition-colors backdrop-blur-sm group flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <div class="w-9 h-9 rounded-lg bg-sky-500/10 border border-sky-500/20 flex items-center justify-center text-sky-400 group-hover:scale-110 transition-transform">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="p-4 rounded-xl bg-slate-900/50 border border-white/5 hover:border-red-500/30 transition-colors backdrop-blur-sm group flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <div class="w-9 h-9 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 group-hover:scale-110 transition-transform">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                                </svg>
                            </div>
                            <span class="text-[10px] font-black text-slate-500">04</span>
                        </div>
                        <h3 class="font-bold text-xs text-white mb-1">
                ...
                <div class="p-4 rounded-xl bg-slate-900/50 border border-white/5 hover:border-red-500/30 transition-colors backdrop-blur-sm group flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <div class="w-9 h-9 rounded-lg bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-purple-400 group-hover:scale-110 transition-transform">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                </svg>
                            </div>
                            <span class="text-[10px] font-black text-slate-500">05</span>
                        </div>
                        <h3 class="font-bold text-xs text-white mb-1">
    ...
    <!-- Footer -->
    <footer class="relative z-10 w-full border-t border-white/10 bg-slate-950/80 backdrop-blur-md py-6">
        <div
            class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400">
            <!-- Left: Legal Info -->
            <div class="flex items-center flex-wrap justify-center sm:justify-start gap-3">
                <span class="font-semibold text-slate-200">MAX PRO SOLS SAS</span>
            ...
            <div class="flex items-center gap-4">
                <a href="{{ url('/admin') }}"
                    class="inline-flex items-center gap-1.5 text-slate-500 hover:text-red-400 transition-colors font-medium text-[11px] py-1 px-2.5 rounded-md hover:bg-white/5">
    ...
    <div x-cloak x-show="contactModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 text-center sm:p-0">
            ...
            <div x-show="contactModalOpen" x-transition:enter="ease-out duration-300"
                ...
                class="relative bg-slate-900 border border-white/10 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full p-6 sm:p-8"
                dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">

                <!-- Close Button -->
                <button type="button" @click="contactModalOpen = false"
                    class="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors p-1 rounded-lg hover:bg-white/5">
                ...
                @if(session('success'))
                    <div
                        class="mb-6 p-4 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-sm font-medium">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <form action="{{ route('contact.submit') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required
                            class="w-full px-3.5 py-2.5 rounded-lg bg-slate-950 border border-white/10 text-white text-sm focus:outline-none focus:border-red-500 transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="subject" required
                            placeholder="{{ app()->getLocale() === 'fr' ? 'Ex: Résine parking 1200m², Parquet boutique...' : 'Ex: Résine, Parquet, Acoustique...' }}"
                            class="w-full px-3.5 py-2.5 rounded-lg bg-slate-950 border border-white/10 text-white text-sm focus:outline-none focus:border-red-500 transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="message" rows="3" required
                            class="w-full px-3.5 py-2.5 rounded-lg bg-slate-950 border border-white/10 text-white text-sm focus:outline-none focus:border-red-500 transition-colors"></textarea>
                    </div>

                    <div class="pt-2">
                        <button type="submit"
                            class="w-full py-3 rounded-lg bg-red-600 hover:bg-red-500 text-white font-bold text-sm shadow-md shadow-red-600/30 transition-all">
