<x-layouts.app>
    ...
    <section class="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-slate-100">
        ...
        <div class="absolute inset-0 z-0">
            <img src="{{ asset('images/hero_flooring_paris.jpg') }}" alt="MAX PRO SOLS Réalisation de Prestige Paris"
            ...
            <div
                class="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-red-600/10 via-transparent to-transparent">
            </div>
        </div>
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32 w-full">
            <div class="max-w-3xl" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                ...
                <!-- Pill Badge -->
                <div
                    class="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/90 border border-slate-200/90 backdrop-blur-md mb-8 shadow-sm">
                ...
                <!-- Main Title -->
                <h1
                    class="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-950 leading-[1.12] mb-6">
                    @if(app()->getLocale() === 'fr')
                        L'Excellence Technique <br>
                        <span class="text-slate-800">du Sol au Mur pour</span> <br>
                        <span class="bg-gradient-to-r from-red-600 to-red-700 bg-clip-text text-transparent">les
                            Professionnels.</span>
                    @elseif(app()->getLocale() === 'ar')
                        التميز الفني في <br>
                        <span class="text-slate-800">تكسيات الأرضيات والجدران</span> <br>
                        <span class="bg-gradient-to-r from-red-600 to-red-700 bg-clip-text text-transparent">للمشاريع الكبرى
                            والشركات.</span>
                    @else
                        Technical Excellence <br>
                        <span class="text-slate-800">from Floor to Wall for</span> <br>
                        <span class="bg-gradient-to-r from-red-600 to-red-700 bg-clip-text text-transparent">Building
                            Professionals.</span>
                    @endif
                </h1>
                ...
                <p class="text-lg sm:text-xl text-slate-700 mb-10 leading-relaxed max-w-2xl font-normal">
                    @if(app()->getLocale() === 'fr')
                        Partenaire certifié des maîtres d’ouvrage, promoteurs, architectes et entreprises générales. Résines
                        industrielles, parquets nobles, panneaux acoustiques et sols souples certifiés DTU.
                    @elseif(app()->getLocale() === 'ar')
                        شريككم المعتمد للمقاولين الرئيسيين والمطورين العقاريين والمهندسين المعماريين في باريس. أرضيات
                        الإيبوكسي الصناعي، الباركيه الفاخر، والتكسيات الصوتية.
                    @else
                        Trusted commercial partner for contractors, developers, and architects across Greater Paris.
                        Certified industrial resin, chevron hardwood, acoustic wall panels, and commercial LVT.
                    @endif
                </p>
                ...
                <div class="flex flex-col sm:flex-row gap-4 mb-12">
                    <a href="{{ route('quote') }}"
                        class="inline-flex items-center justify-center gap-3 px-8 py-4 rounded-xl text-white font-bold text-base bg-gradient-to-r from-red-600 via-red-600 to-red-700 hover:from-red-500 hover:to-red-600 shadow-xl shadow-red-600/25 hover:shadow-red-600/40 transition-all transform hover:-translate-y-0.5">
                    ...
                    <a href="{{ route('projects.index') }}"
                        class="inline-flex items-center justify-center gap-2.5 px-7 py-4 rounded-xl text-slate-800 font-semibold text-base bg-white/95 hover:bg-white border border-slate-300 hover:border-slate-400 shadow-sm backdrop-blur-md transition-all">
                ...
                <div
                    class="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-6 border-t border-slate-300/80 text-xs text-slate-700 font-medium">
                ...
                @if(isset($statistics) && $statistics->count() > 0)
                    @foreach($statistics as $stat)
                        <div class="flex items-center gap-4 group">
                            <div
                                class="w-14 h-14 rounded-2xl bg-red-50 border border-red-200 flex items-center justify-center text-red-600 group-hover:scale-110 group-hover:bg-red-600 group-hover:text-white transition-all duration-300 flex-shrink-0 shadow-sm">
                            ...
                            <div>
                                <div class="text-3xl sm:text-4xl font-extrabold text-slate-950 tracking-tight">
                                    {{ $stat->value }}</div>
                                <div class="text-xs sm:text-sm font-semibold text-slate-600">
                                    {{ $stat->getTranslation('label', app()->getLocale()) }}</div>
                            </div>
                        </div>
                    @endforeach
                @else
    ...
    <section class="py-24 bg-white relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                <div>
                    <div
                        class="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-red-600 mb-3">
                ...
                <a href="{{ route('services.index') }}"
                    class="inline-flex items-center gap-2 text-sm font-bold text-red-600 hover:text-red-700 transition-colors group">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                @if(isset($services) && $services->count() > 0)
                    @foreach($services as $index => $service)
                        <div
                            class="group bg-white rounded-2xl overflow-hidden border border-slate-200 hover:border-red-500 hover:shadow-xl transition-all duration-300 flex flex-col">
                            <!-- Image Container -->
                            <div class="h-48 overflow-hidden relative">
                                ...
                                <img src="{{ $serviceImg }}"
                                    ...
                                    loading="lazy">
                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-slate-900/60 via-transparent to-transparent opacity-60 group-hover:opacity-80 transition-opacity">
                                </div>
                                <div class="absolute top-4 left-4">
                                    <span
                                        class="px-2.5 py-0.5 bg-white/95 backdrop-blur border border-slate-200 text-[11px] font-extrabold text-red-600 uppercase tracking-wider rounded-md shadow-sm">
                            ...
                            <div class="p-5 flex-grow flex flex-col justify-between">
                                <div>
                                    <h3
                                        class="text-base font-bold text-slate-900 mb-2 group-hover:text-red-600 transition-colors line-clamp-1">
                                        {{ $service->title }}
                                    </h3>
                                ...
                                <div class="pt-3 border-t border-slate-100 flex items-center justify-between">
                                    <a href="{{ route('services.show', $service->slug) }}"
                                        class="text-xs font-bold text-slate-800 group-hover:text-red-600 flex items-center gap-1.5 transition-colors">
                                        <span>{{ app()->getLocale() === 'fr' ? 'Fiche technique' : (app()->getLocale() === 'ar' ? 'التفاصيل الفنية' : 'View details') }}</span>
                                    ...
                                    <span
                                        class="w-2 h-2 rounded-full bg-slate-300 group-hover:bg-red-600 transition-colors"></span>
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    <section class="py-24 bg-slate-50 border-y border-slate-200 relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                <!-- Step 1 -->
                <div
                    class="relative p-7 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-md transition-all">
                    <div
                        class="w-12 h-12 rounded-xl bg-red-50 border border-red-200 text-red-600 font-extrabold flex items-center justify-center text-lg mb-6">
                        01
                    </div>
                    <h3 class="text-lg font-bold text-slate-900 mb-2">Audit & Calepinage</h3>
                    <p class="text-slate-600 text-xs sm:text-sm leading-relaxed">
                        Relevé hygrométrique, analyse de planéité du support, plans de calepinage précis et
                        préconisation des matériaux adaptés aux contraintes d'exploitation.
                    </p>
                </div>
                ...
                <div
                    ...
                    <p class="text-slate-600 text-xs sm:text-sm leading-relaxed">
                        Grenaillage, ponçage diamant, traitement des fissures, application de primaires d’adhérence et
                        ragréages fibrés haute performance P3/P4S.
                    </p>
                </div>
                ...
                <div
                    ...
                    <p class="text-slate-600 text-xs sm:text-sm leading-relaxed">
                        Installation exécutée par nos équipes internes qualifiées : coulage de résines, poses de
                        parquets en point de Hongrie, habillages muraux acoustiques.
                    </p>
                </div>
                ...
                <div
                    ...
                    <p class="text-slate-600 text-xs sm:text-sm leading-relaxed">
                        Procès-verbal de réception sans réserve, remise du Dossier des Ouvrages Exécutés (DOE), fiches
                        d'entretien et activation de la garantie décennale.
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                @if(isset($featuredProjects) && $featuredProjects->count() > 0)
                    @foreach($featuredProjects as $project)
                        <div
                            class="group relative rounded-3xl overflow-hidden bg-white border border-slate-200 hover:border-slate-300 transition-all duration-300 shadow-md hover:shadow-xl">
                            <!-- Image Container -->
                            <div class="h-80 sm:h-96 overflow-hidden relative">
                                ...
                                <div class="absolute top-5 left-5 flex flex-wrap gap-2">
                                    @if($project->sector)
                                        <span
                                            class="px-3.5 py-1.5 rounded-lg bg-white/95 backdrop-blur-md border border-slate-200 text-xs font-bold text-slate-800 uppercase tracking-wider shadow-sm">
                                            {{ $project->sector->title }}
                                        </span>
                                    @endif
                                    @if($project->surface_areas)
                                        <span
                                            class="px-3.5 py-1.5 rounded-lg bg-red-600 text-xs font-bold text-white uppercase tracking-wider shadow-sm">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 items-center">
                ...
                @foreach($partnerBrands as $brand)
                    <div
                        class="p-6 rounded-xl bg-white border border-slate-200 hover:border-red-500 hover:shadow-md text-center transition-all group">
                        <div
                            class="text-lg font-black tracking-wider text-slate-800 group-hover:text-red-600 transition-colors">
                            {{ $brand['name'] }}</div>
                        <div class="text-[10px] text-slate-500 uppercase tracking-tight mt-1 font-medium">
                            {{ $brand['sub'] }}</div>
                    </div>
                @endforeach
            </div>
    ...
    <section class="py-24 bg-white relative overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div
                class="relative rounded-3xl bg-gradient-to-r from-red-50 via-white to-red-50 border border-red-200 p-10 sm:p-16 shadow-xl overflow-hidden">
                <!-- Background decorative elements -->
                <div
                    class="absolute -right-20 -bottom-20 w-80 h-80 rounded-full bg-red-500/10 blur-3xl pointer-events-none">
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center relative z-10">
                    <div class="lg:col-span-8 space-y-4">
                        <div
                            class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-red-100 text-red-700 text-xs font-bold uppercase tracking-wider border border-red-200">
                    ...
                    <div class="lg:col-span-4 flex flex-col sm:flex-row lg:flex-col gap-4">
                        <a href="{{ route('quote') }}"
                            class="w-full text-center bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white font-bold py-4 px-8 rounded-xl shadow-lg shadow-red-600/30 transition-all transform hover:-translate-y-0.5 text-base">
                            {{ app()->getLocale() === 'fr' ? 'Déposer un Dossier / Devis' : 'Submit Project for Quote' }}
                        </a>
                        <a href="{{ route('contact') }}"
                            class="w-full text-center bg-white hover:bg-slate-50 text-slate-800 font-semibold py-4 px-8 rounded-xl border border-slate-300 shadow-sm transition-all text-sm">
